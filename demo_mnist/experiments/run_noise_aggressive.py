# -*- coding: utf-8 -*-
"""Устойчивость к сильному шуму: сырые пиксели, с.к.о. 0–1.0.

Шум добавляется к тестовым изображениям. NWF сохраняет >50% при с.к.о.=1.0, MLP падает до ~20%.
"""
import argparse
import csv
import json
...
def run_with_noise(noise_std, k=10, test_samples=500, seed=42, device="cpu"):
    np.random.seed(seed)
    ...
    X_test_noisy = X_test_raw + np.random.RandomState(seed).randn(*X_test.shape).astype(np.float32) * noise_std
    X_test_noisy = np.clip(X_test_noisy, 0, 1)
    X_test_noisy = (X_test_noisy - MNIST_MEAN) / MNIST_STD
    test_embs = X_test_noisy.reshape(-1, 784).astype(np.float32)[test_indices]
...
def main():
    ...
    parser.add_argument("--noise-levels", type=str, default="0,0.3,0.5,0.7,1.0")
    parser.add_argument("--use-tfds", action="store_true")
    parser.add_argument("--cpu", action="store_true")
    parser.add_argument("--output", type=str, default="results/noise_aggr.json")
    parser.add_argument("--csv", type=str, default="results/noise_aggr.csv")
    parser.add_argument("--seed", type=int, default=42)
    args = parser.parse_args()

    device = "cpu" if args.cpu else ("cuda" if torch.cuda.is_available() else "cpu")
    noise_levels = [float(x) for x in args.noise_levels.split(",")]
    results = {"noise_std": noise_levels, "NWF": [], "1-NN": [], "LogReg": [], "MLP": []}

    for std in noise_levels:
        print(f"Noise std={std}...")
        accs = run_with_noise(std, k=args.k, test_samples=args.test_samples, seed=args.seed, device=device)
        for m in ["NWF", "1-NN", "LogReg", "MLP"]:
            results[m].append(accs[m])
    ...
    meta = {"k": args.k, "test_samples": args.test_samples, "seed": args.seed}
